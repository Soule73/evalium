...
const REHYPE_PLUGINS: PluggableList = [rehypeKatex, rehypePrismPlus];
...
const MARKDOWN_COMPONENTS: Components = {
    ...
    h3: ({ children }) => (
        <h3 className="text-lg font-medium text-gray-800 mb-2 mt-4 first:mt-0">{children}</h3>
    ),
    h4: ({ children }) => (
        <h4 className="text-base font-medium text-gray-700 mb-2 mt-3 first:mt-0">{children}</h4>
    ),
    h5: ({ children }) => (
        <h5 className="text-sm font-medium text-gray-700 mb-1 mt-3 first:mt-0">{children}</h5>
    ),
    h6: ({ children }) => (
        <h6 className="text-sm font-medium text-gray-600 mb-1 mt-3 first:mt-0">{children}</h6>
    ),

    p: ({ children }) => <p className="text-gray-700 mb-4 leading-relaxed last:mb-0">{children}</p>,

    ul: ({ children }) => (
        <ul className="list-disc list-inside text-gray-700 mb-4 space-y-1 ml-4">{children}</ul>
    ),
    ol: ({ children }) => (
        <ol className="list-decimal list-inside text-gray-700 mb-4 space-y-1 ml-4">{children}</ol>
    ),
    ...
    td: ({ children }) => <td className="px-4 py-2 text-sm text-gray-700 border-b">{children}</td>,
    ...
    img: ({ src, alt }) => (
        <div className="mb-4">
            <img src={src} alt={alt} className="max-w-full h-auto rounded-lg shadow-sm border" />
