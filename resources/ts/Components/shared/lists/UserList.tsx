...
export function UserList({
    ...
    const handleToggleStatus = useCallback((userId: number) => {
        router.patch(
            route('admin.users.toggle-status', { user: userId }),
            {},
            {
                preserveScroll: true,
            },
    ...
    const handleRestoreUser = useCallback((userId: number) => {
        router.post(
            route('admin.users.restore', { id: userId }),
            {},
            {
                preserveScroll: true,
            },
            ...
            },
            {
                ...
                render: (user: User) =>
                    (user?.roles?.length ?? 0) > 0 ? (
                        <span
                            className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getRoleColor(user.roles?.[0]?.name ?? '')}`}
                        >
                ...
                render: (user: User) => (
                    <div className="flex items-center gap-2">
                        {user.deleted_at ? (
                            ...
                            <span
                                className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${user.is_active ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}
                            >
                                {user.is_active
                            ...
                            {user.deleted_at ? (
                                <>
                                    ...
                                    {permissions.canDelete && onForceDeleteClick && (
                                        <Button
                                            onClick={() =>
                                                onForceDeleteClick({ id: user.id, name: user.name })
                                            }
                                            color="danger"
                                            size="sm"
                                            variant="outline"
                                ...
                                <>
                                    ...
                                    {permissions.canDelete &&
                                        !canViewUser(userRole) &&
                                        onDeleteClick && (
                                            <Button
                                                onClick={() =>
                                                    onDeleteClick({ id: user.id, name: user.name })
                                                }
                                                color="danger"
                                                size="sm"
                                                variant="outline"
                    ...
                    ? [
                          {
                              ...
                              options: [
                                  { label: t('admin_pages.users.all_roles'), value: '' },
                              ].concat(
