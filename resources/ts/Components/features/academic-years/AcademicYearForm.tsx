...
export default function AcademicYearForm({
  ...
  const handleChange = useCallback((field: keyof Omit<AcademicYearFormData, 'semesters'>, value: string | boolean) => {
    ...
    if ((field === 'start_date' || field === 'end_date') && next.start_date && next.end_date) {
      ...
      if (!hasUserEdited) {
        next.semesters = buildDefaultSemesters(next.start_date as string, next.end_date as string);
      }
    }
  ...
  const handleSemesterChange = useCallback((index: number, field: keyof SemesterFormData, value: string) => {
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-6">
      ...
      <Section
        ...
        actions={
          <Button type="button" size="sm" variant="outline" color="primary" onClick={addSemester}>
      ...
      >
        ...
        <div className="space-y-4">
          {formData.semesters.map((semester, index) => (
            <div key={semester.id ?? `new-${index}`} className="border border-gray-200 rounded-lg p-4 bg-gray-50">
              <div className="flex items-center justify-between mb-3">
                <h4 className="text-sm font-medium text-gray-700">
                  {t('admin_pages.academic_years.semester_number', { number: index + 1 })}
                </h4>
