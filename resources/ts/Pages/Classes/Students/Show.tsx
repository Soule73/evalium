...
/**
 * Shared page displaying a student's grade breakdown within a class context.
...
export default function ClassStudentShow({
    ...
    const canUpdate = isAdmin && hasPermission(auth.permissions, 'update enrollments');
    ...
    const pageBreadcrumbs = useMemo(
        () =>
            isAdmin
                ...
                : breadcrumbs.teacher.showClassStudent(classItem, enrollment),
        [breadcrumbs, classItem, enrollment, isAdmin],
    );

    const backHref = routeContext.studentIndexRoute
        ? route(routeContext.studentIndexRoute, classItem.id)
        : route(routeContext.showRoute, classItem.id);
    ...
    const handleSubjectClick = useCallback(
        (subject: SubjectGrade) => {
            if (!routeContext.studentAssignmentsRoute) return;
            router.visit(
                route(routeContext.studentAssignmentsRoute, {
                    class: classItem.id,
        ...
        [classItem.id, enrollment.id, routeContext.studentAssignmentsRoute],
    );
                ...
                <Section
                    ...
                    actions={
                        <div className="flex space-x-3">
                            ...
                            {routeContext.studentAssignmentsRoute && (
                                <Button
                                    size="sm"
                                    variant="outline"
                                    color="primary"
