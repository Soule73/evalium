...
export default function EnrollmentCreate({ classes, students: initialStudents }: Props) {
  ...
  const handleStudentCreated = useCallback((newStudent: Pick<User, 'id' | 'name' | 'email' | 'avatar'>) => {
    const studentUser = newStudent as User;
    setStudents((prev) => [...prev, studentUser].sort((a, b) => a.name.localeCompare(b.name)));
  ...
  const isStudentAlreadyEnrolled = selectedStudent && enrolledStudents.some(
  ...
  const capacityPercentage = selectedClass
    ? Math.round(((selectedClass.active_enrollments_count || 0) / selectedClass.max_students) * 100)
    : 0;

  const capacityColor = capacityPercentage >= 100
    ? 'bg-red-500'
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <>
                  <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
                    <div className="mb-3 flex items-center justify-between">
                      <span className="text-sm font-medium text-gray-700">{translations.capacityLabel}</span>
                      <span className={`text-sm font-semibold ${availableSlots > 0 ? 'text-gray-900' : 'text-red-600'}`}>
                        {selectedClass.active_enrollments_count || 0} / {selectedClass.max_students}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center justify-between text-xs text-gray-500">
                      <span>{translations.availableSlots}: {availableSlots}</span>
                  ...
                  <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
                    <h4 className="mb-3 text-sm font-medium text-gray-700">
                      {translations.enrolledStudents} ({enrolledStudents.length})
                    </h4>
                    {enrolledStudents.length === 0 ? (
                      <p className="text-xs text-gray-400 italic">{translations.noEnrolledStudents}</p>
                    ) : (
          ...
          <div className="flex justify-end space-x-3 pt-6">
            ...
            <Button
              ...
              variant="solid"
              color="primary"
              disabled={isSubmitting || availableSlots === 0 || !!isStudentAlreadyEnrolled}
            >
