...
export default function EnrollmentAssignmentsIndex({ enrollment, assignments, subjects, filters }: Props) {
  const { t } = useTranslations();
  const breadcrumbs = useBreadcrumbs();

  const getStatusBadge = useCallback((status: string) => {
    const statusMap: Record<string, { label: string; type: 'gray' | 'info' | 'warning' | 'success' }> = {
      not_submitted: { label: t('components.assignment_list.status_not_started'), type: 'gray' },
      in_progress: { label: t('components.assignment_list.status_in_progress'), type: 'info' },
      submitted: { label: t('components.assignment_list.status_pending_grading'), type: 'warning' },
    ...
    columns: [
      {
        ...
        render: (item: AssessmentAssignment) => (
          <div>
            <div className="font-medium text-gray-900">{item.assessment?.title}</div>
      ...
      },
      {
        ...
        render: (item: AssessmentAssignment) => {
          ...
          const percentage = maxPoints > 0 ? Math.round((Number(item.score) / maxPoints) * 100) : 0;
          return (
            <div>
              <div className="text-sm font-medium text-gray-900">{item.score} / {maxPoints}</div>
      ...
      },
      {
        ...
        render: (item: AssessmentAssignment) =>
          item.submitted_at
            ? <span className="text-sm text-gray-600">{formatDate(item.submitted_at, 'datetime')}</span>
      ...
      },
      {
        ...
        render: (item: AssessmentAssignment) => (
          <Button
            ...
            color="secondary"
            onClick={() =>
              router.visit(route('admin.enrollments.assignments.show', {
        ...
        <Section
          ...
          actions={
            <Button
              ...
              variant="outline"
              color="secondary"
              onClick={() => router.visit(route('admin.enrollments.show', enrollment.id))}
            >
