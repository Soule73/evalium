...
import { Assessment, AssessmentAssignment, Answer, User, Question, PageProps, QuestionResult, Choice } from '@/types';
import { requiresManualGrading } from '@/utils';
import { route } from 'ziggy-js';
import { router, usePage } from '@inertiajs/react';
import { Badge, Button, Section, Textarea } from '@/Components';
...
const GradingShow: React.FC<Props> = ({ assessment, student, assignment, userAnswers = {} }) => {
  ...
  const handleConfirmSubmit = useCallback(() => {
    ...
    router.post(route('teacher.grading.save', { assessment: assessment.id, student: student.id }), {
      scores: scoresWithFeedback,
      teacher_notes: teacherNotes
    }, {
      onFinish: () => setIsSubmitting(false)
    });
  }, [assessment, student, scores, feedbacks, teacherNotes]);
      ...
      <div className="max-w-6xl mx-auto space-y-6">
          ...
          actions={
            <div className="flex items-center space-x-4">
              <Button
                ...
                size="sm"
              >
                {trans('grading_pages.show.back_to_grading')}
              </Button>
              {canGradeAssessments && (
                <Button
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 bg-blue-50 rounded-lg">
            <div>
              <p className="text-sm text-gray-600">{trans('grading_pages.show.total_score')}</p>
              <p className="text-2xl font-bold text-gray-900">{calculatedTotalScore} / {totalPoints}</p>
            </div>
            <div>
              <p className="text-sm text-gray-600">{trans('grading_pages.show.percentage')}</p>
              <p className="text-2xl font-bold text-gray-900">{percentage}%</p>
            </div>
            <div>
              <p className="text-sm text-gray-600">{trans('grading_pages.show.status')}</p>
              <Badge
                label={assignment.submitted_at ? 'Submitted' : 'Not Submitted'}
