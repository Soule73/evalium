...
import { ClassSubject, AssessmentFormData, QuestionFormData, ChoiceFormData } from '@/types';
...
export default function Create({ classSubjects }: Props) {
  const { data, setData, post, processing, errors } = useForm<AssessmentFormData>({
    class_subject_id: 0,
    title: '',
    description: '',
    type: 'devoir',
  ...
  const handleRemoveQuestion = (index: number) => {
    ...
    setData('questions', newQuestions.map((q, i) => ({ ...q, order_index: i })));
  ...
  return (
    <AuthenticatedLayout
      title={trans('teacher_pages.assessments.create.title')}
      breadcrumb={breadcrumbs.createTeacherAssessment()}
    >
      <Section
        title={trans('teacher_pages.assessments.create.heading')}
        subtitle={trans('teacher_pages.assessments.create.description')}
      >
        <form onSubmit={handleSubmit} className="space-y-8">
          <div className="bg-white rounded-lg border border-gray-200 p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-6">
              {trans('teacher_pages.assessments.create.basic_info')}
            </h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="md:col-span-2 flex items-center">
                ...
                <label htmlFor="is_published" className="ml-2 text-sm text-gray-700">
                  {trans('teacher_pages.assessments.form.publish_immediately')}
                </label>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-6">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-lg font-semibold text-gray-900">
                {trans('teacher_pages.assessments.create.questions_section')}
              </h3>
              <Button
                type="button"
              ...
              >
                ...
                {trans('teacher_pages.assessments.create.add_question')}
              </Button>
            </div>

            {data.questions && data.questions.length === 0 && (
              <div className="text-center py-12 text-gray-500">
                <p className="mb-4">{trans('teacher_pages.assessments.create.no_questions')}</p>
                <Button
                  type="button"
                  onClick={handleAddQuestion}
                  color="primary"
                ...
                >
                  <PlusIcon className="w-4 h-4 mr-1" />
                  {trans('teacher_pages.assessments.create.add_first_question')}
                </Button>
              </div>
            )}
                ...
                <div key={qIndex} className="border border-gray-200 rounded-lg p-4">
                  <div className="flex justify-between items-start mb-4">
                    <h4 className="font-medium text-gray-900">
                      {trans('teacher_pages.assessments.create.question_number', { number: qIndex + 1 })}
                    </h4>
                    <div className="flex gap-2">
                      ...
                      >
                        {expandedQuestions.has(qIndex)
                          ? trans('teacher_pages.assessments.create.collapse')
                          : trans('teacher_pages.assessments.create.expand')
                        }
                      </Button>
                      <Button
                        type="button"
                      ...
                      {(question.type === 'one_choice' || question.type === 'multiple' || question.type === 'boolean') && (
                        <div className="mt-4">
                          <div className="flex justify-between items-center mb-3">
                            ...
                            >
                              <PlusIcon className="w-4 h-4 mr-1" />
                              {trans('teacher_pages.assessments.create.add_choice')}
                            </Button>
                          </div>
          ...
          <div className="flex justify-end gap-4">
            ...
            >
              {processing
                ...
                : trans('teacher_pages.assessments.create.submit')
              }
            </Button>
          </div>
        </form>
