...
import { useListGroups } from '@/hooks/features/groups';
import { getBooleanStatusInfo } from '@/utils/formatting/formatters';
    ...
    const dataTableConfig: DataTableConfig<Group> = {
        columns: [
            ...
            },
            {
                ...
                render: (group) => {
                    const statusInfo = getBooleanStatusInfo(group.is_active);
                    return (
    ...
    return (
        ...
        >
            ...
            <ConfirmationModal
                ...
                title={bulkModal.data === 'activate' ? trans('admin_pages.groups.bulk_activate_title') : trans('admin_pages.groups.bulk_deactivate_title')}
                message={
                    bulkModal.data === 'activate'
                        ? trans('admin_pages.groups.bulk_activate_message', { count: selectedIds.length })
                        : trans('admin_pages.groups.bulk_deactivate_message', { count: selectedIds.length })
                }
                confirmText={trans('admin_pages.common.confirm')}
                cancelText={trans('admin_pages.common.cancel')}
                type={bulkModal.data === 'activate' ? 'info' : 'warning'}
                icon={bulkModal.data === 'activate' ? CheckCircleIcon : XCircleIcon}
                loading={isLoading}
            />
        </AuthenticatedLayout>
    );
