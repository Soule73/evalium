...
const AssessmentShow: React.FC<Props> = ({ assessment, assignments, routeContext }) => {
  ...
  const handleToggleStatus = () => {
    ...
    setIsToggling(true);
    const toggleRoute = assessment.is_published ? routeContext!.unpublishRoute! : routeContext!.publishRoute!;
    router.post(route(toggleRoute, assessment.id), {}, {
      preserveScroll: true,
  ...
  const handleDuplicate = () => {
    ...
    router.post(route(routeContext!.duplicateRoute!, assessment.id), {}, {
      onFinish: () => {
  ...
  const handleGradeStudent = (assignment: AssignmentWithVirtual) => {
    ...
    router.visit(route(routeContext.gradeRoute, {
  ...
  const handleViewResult = (assignment: AssignmentWithVirtual) => {
    if (!assignment.id || !routeContext) return;
    router.visit(route(routeContext.reviewRoute, {
      assessment: assessment.id,
      assignment: assignment.id,
    }));
  };
