<?php

declare(strict_types=1);

return [
    'page_titles' => [
        'index' => 'Assessments',
    ],
    'index' => [
        'title' => 'Assessment Management',
        'subtitle' => 'Create, manage and assign your assessments to classes.',
        'new_assessment' => 'New Assessment',
    ],
    'create' => [
        'title' => 'Create Assessment',
        'subtitle' => 'Configure your assessment settings and add your questions.',
        'cancel' => 'Cancel',
        'submit' => 'Create Assessment',
    ],
    'edit' => [
        'title' => 'Edit Assessment',
        'subtitle' => 'Modify your assessment settings and questions.',
        'cancel' => 'Cancel',
        'submit' => 'Save Changes',
    ],
    'common' => [
        'subtitle' => 'View and manage your assessment.',
        'duplicate' => 'Duplicate',
        'edit' => 'Edit',
        'toggle_published' => 'Published',
        'toggle_unpublished' => 'Unpublished',
        'questions' => 'Questions',
        'total_points' => 'Total Points',
        'duration' => 'Duration',
        'concerned_students' => 'Concerned Students',
        'assessment_questions' => 'Assessment Questions',
        'no_questions' => 'No questions added yet',
        'add_questions' => 'Add Questions',
        'free_text_info' => 'Free text answer expected',
        'duplicate_modal_title' => 'Duplicate Assessment',
        'duplicate_modal_message' => 'Create a copy of ":title"?',
        'duplicate_confirm' => 'Duplicate',
        'class' => 'Class',
        'subject' => 'Subject',
        'level' => 'Level',
        'teacher' => 'Teacher',
        'share_with_students' => 'Share with students',
        'share_modal_title' => 'Share assessment',
        'share_modal_instruction' => 'Ask students to enter this ID in the "Quick access" field in their sidebar.',
        'share_modal_id_label' => 'Assessment ID',
        'share_modal_type' => 'Type',
        'share_modal_mode' => 'Mode',
        'share_modal_duration' => 'Duration',
        'share_modal_questions' => 'Questions',
        'share_modal_points' => 'Points',
        'share_modal_close' => 'Close',
    ],
    'show' => [
        'students_section_title' => 'Students',
        'stats_section_title' => 'Completion Progress',
        'students_section_subtitle' => ':count student(s) concerned',
        'student' => 'Student',
        'stats_graded' => 'Graded',
        'stats_in_progress' => 'In Progress',
        'stats_not_started' => 'Not Started',
        'stats_average_score' => 'Average grade',
        'status_chart' => 'Assignment status',
        'status_chart_subtitle' => 'Visual breakdown of student assignment statuses',
        'score_distribution' => 'Score distribution',
        'score_distribution_subtitle' => 'Student scores normalized to /20',
    ],
];
