...
|---|----------|-------------|-----|
| 1 | High | DB unique `(class_id, student_id)` blocks re-enrollment after withdrawal / back-transfer | `Enrollment::create()` → `Enrollment::updateOrCreate()` in `enrollStudent()` and `transferStudent()` |
| 2 | High | `StudentEnrollmentController` injected concrete `EnrollmentService` instead of interface | Changed type hint to `EnrollmentServiceInterface` |
| 3 | High | `getEnrollmentHistory()` param `int $academicYearId` but called with `?int` → TypeError | Changed to `?int` + `->when()` guard |
| 4 | High | `EnrollmentRepository::getEnrollmentsForIndex()` called `->forAcademicYear($academicYearId)` without null guard + same in `getShowData()` | Wrapped in `->when($academicYearId, ...)` throughout the repo |
| 5 | Medium | 7 hardcoded `'active'` / `'withdrawn'` strings bypassing `EnrollmentStatus` enum | Replaced with `EnrollmentStatus::Active` / `EnrollmentStatus::Withdrawn` in all 3 files |
| 6 | Medium | `resolvePreviousAcademicYear()` was a private business query method in the controller (SRP) | Moved to `EnrollmentRepository`, added to `EnrollmentRepositoryInterface` |
| 7 | Medium | `searchClasses()` called `ClassModel::forAcademicYear($selectedYearId)` without null guard | Replaced with `ClassModel::query()->when($selectedYearId, ...)` |
| 8 | Medium | `StudentEnrollmentRepository` had no interface — inconsistent DI pattern | Created `StudentEnrollmentRepositoryInterface`, bound in `AppServiceProvider` |
| 9 | Low | `EnrollmentPolicy` missing class PHPDoc | Added PHPDoc block |
| 10 | Low | `bulkStore()` used FQCN `\App\Notifications\UserCredentialsNotification` inline despite existing import | Removed FQCN, uses imported class |

**Tests:** 847/847 passing after all fixes.
